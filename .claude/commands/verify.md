...
```bash
# 노드 상태
vagrant ssh master -c "kubectl get nodes" 2>/dev/null || echo "VM not running"

# Pod 상태
vagrant ssh master -c "kubectl get pods -A --field-selector=status.phase!=Running 2>/dev/null | head -20" || true

# ArgoCD 앱 상태
vagrant ssh master -c "kubectl get applications -n argocd 2>/dev/null" || true
```
...
[OK] scripts/master/01-init-cluster.sh
[WARN] scripts/master/05-keycloak.sh - shellcheck warnings
