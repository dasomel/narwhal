...
| 2026-06-07 | Master 4GB still too tight at steady state: apiserver ~2GB (no mem limit) + mandatory DaemonSets ~0.8GB → ~567Mi headroom on master-3 → apiserver SIGKILL (exit 137 via /livez timeout) + intermittent NodeNotReady flapping (also broke metrics-server → portal CPU/mem blank) | Diagnosed only (see `docs/master-memory-pressure.md`). Durable fix: bump `MASTER_MEMORY` 4096→6144 in Vagrantfile, rolling `vagrant reload` one master at a time. Partial: exclude falco DaemonSet from masters |
