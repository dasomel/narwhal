...
def high(paths,policy):
  return [p for p in paths if "high" in policy.get("traceRequiredAt",[]) and any(r.get("risk")=="high" and fnmatch.fnmatch(p,r["pattern"]) for r in policy.get("rules",[]))]
