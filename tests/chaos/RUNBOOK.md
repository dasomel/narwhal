...
| :--- | :--- | :--- | :--- |
| 2026-07-17 14:01 KST | Claude/Fable | **PASS** | istiod 2 replica라 SPOF 미발생 — 실험 내내 포털 200·grafana 302 무중단, mesh 트래픽 끊김 0. 기존 세션 트래픽은 ztunnel이 계속 처리, istiod는 신규 enrollment/config 배포만 담당함을 확인. |
...
| 실험 일시 | 실행자 | 결과 (PASS/FAIL) | 특이사항 / 관측 지표 |
| :--- | :--- | :--- | :--- |
| 2026-07-17 14:00 KST | Claude/Fable | **PASS** | kill 직후 reseal(readiness false, ~13:53:03) → auto-unseal CronJob(2분 주기)이 ~66초 만에 unseal 복구(13:54:09 ready). 복구 창(3분) 내 자동 회복, 수동 개입 불필요. 최초 실행은 auto-unseal Job 파드(0/1 Running)를 오집계해 FAIL → 하네스를 selector 기반 판정으로 수정 후 재실행 PASS. |
...
| 실험 일시 | 실행자 | 결과 (PASS/FAIL) | 특이사항 / 관측 지표 |
| :--- | :--- | :--- | :--- |
| 2026-07-17 14:06 KST | Claude/Fable | **PASS** | mode:one이라 coredns 2개 중 1개만 500ms 지연 → 나머지 replica가 쿼리 흡수, 포털 200 무영향. DNS 이중화가 단일 파드 지연을 완전 격리함을 확인. |
...
| 실험 일시 | 실행자 | 결과 (PASS/FAIL) | 특이사항 / 관측 지표 |
| :--- | :--- | :--- | :--- |
| 2026-07-17 13:52 KST | Claude/Fable | **PASS** | 가설 확정: kill 전 확보한 cluster-admin 세션이 Keycloak realm 502(약 40초) 구간에도 계속 cluster-admin 유지 — 포털은 자체 JWT 검증, Keycloak 재히트 안 함. Keycloak도 ~40초 만에 복구. 신규 로그인만 그 구간 실패. |
...
| 실험 일시 | 실행자 | 결과 (PASS/FAIL) | 특이사항 / 관측 지표 |
| :--- | :--- | :--- | :--- |
| 2026-07-17 14:08 KST | Claude/Fable | **PASS** | worker-3 CPU 7%→83%(60초)→8% 정상 상승/회복. **어떤 파드도 프로브킬 안 됨**(재시작 변화 0, not-ready 0 유지). 단일 노드 83%/60초로는 2026-07-14 프로브킬 폭풍이 재현되지 않음 → 그 사건이 **단일 노드가 아닌 호스트 전체 과다구독**(Docker kind가 6 VM 동시 기아)이었다는 RCA를 역으로 입증. |
...
| 실험 일시 | 실행자 | 결과 (PASS/FAIL) | 특이사항 / 관측 지표 |
| :--- | :--- | :--- | :--- |
| 2026-07-17 14:06 KST | Claude/Fable | **PASS** | CNPG 자동 failover 우수: primary가 narwhal-db-1 → narwhal-db-2로 **8초 내 전환**(14:06:46→14:06:54), 전환 중 Keycloak realm 내내 200 — 인증 서비스 무중단. |
